...
!(function e(t, n, r) {
  ...
  1: [function (require, module, exports) {
    $('.slider-arrow').click(function () {
      $(this).hasClass('show') ? ($(window).width() <= 700 ? $('.slider-arrow, .sidebar, .sidebar-legend').animate({ left: '+=80%' }, 200, () => {
        ...
        .addClass('fa fa-angle-left')) : ($(window).width() <= 700 ? $('.slider-arrow, .sidebar, .sidebar-legend').animate({ left: '-=80%' }, 200, () => {
      }) : $('.slider-arrow, .sidebar, .sidebar-legend').animate({ left: '-=483' }, 200, () => {
      }), $(this).removeClass('hide').addClass('show').find('i')
        .removeClass('fa fa-angle-left')
        .addClass('fa fa-angle-right'));
    }), $html = $('html'), $pins2 = [], $pins = [], $location = null, $radius = null, $me = null, $nearbyPins = [], $markers = [], $radiusControl = $('#filters input[type="range"]'), $map = null, $geocoder = null, $filters = $('.sidebar-content #filter-options input[type="checkbox"]'), $consolidateFilters = window.locatorConfig.consolidateFilters, $viewingPin = null, $pinIcon = 'pin.svg', $meIcon = '/blocks/dealer-locator/images/ME.svg', $panes = [], $lastPane = '', $radiusValue = $('#range').val(), $sortedPins = null, $offset = (new Date()).getTimezoneOffset() / 60 * -1, $key = 'AIzaSyAP8IewqHuU8SMz_6tNiIUlbU_l0GFOd1w', $myDealer = null, $wayPoints = [], $directionsService = null, $directionsDisplay = null, $directionsObject = null, $printDirections = $('#print-directions'), $printableDirections = $('#printable-directions'), $directionResults = null, $currentAddress = '', $directionsMessage = $('#directions-message'), $isAsist = window.locatorConfig.asist, $isAmentities = window.locatorConfig.amenities, $brandOptionSelected = '', $eloquaFormHTML = $('script#eloquaForm').html(), $showAsistDialog = window.locatorConfig.showAsistDialog;
    $electricDealer = !1, $hoverText = $('#hoverText').val(), (function () {
      initMap = function () {
        if ($geocoder = new google.maps.Geocoder(), $map = new google.maps.Map(document.getElementById('map'), {
          ...
          $(options).on('click', (ev) => {
            ...
            $.fn.loadPins(), $('.datasource-option').toggle();
          });
        }
      };
      ...
      $pins = [], $nearbyPins = [], $markers = [], window.locatorConfig.dataSource || (window.locatorConfig.dataSource = '/simpleprox.ashx?https://mvservices.na.volvogroup.com/Dualbrand_US_DealerJSON.ashx'), $.ajax({
        ...
        success(data) {
                  ...
                  for (const dealer in $state.dealers) {
                    if ($state.dealers.hasOwnProperty(dealer)) {
                      ...
                      $marker = new google.maps.Marker({
                        ...
                        const index = $markers.indexOf(this); const
          ...
          const markerId = $.fn.getUrlParameter('view'); const
      ...
      let hours = $.fn.getHours(dealer); const zone = moment.tz.guess(); let
        closeSoon = !1;
      if (hours) {
        time || (time = new Date());
      ...
      let markerDetails; var templateClone = $($('#sidebar-directions').clone(!0).html());
      ...
      const isOpen = $.fn.isOpen(markerDetails); let
        isOpenHtml = '';
      ...
      const servicesHtml = templateClone.find('#services'); const driversHtml = templateClone.find('#drivers');
      const driverAmenities = []; const newdriverAmenities = []; const
        serviceAmenities = [];
      ...
      let hours = null; const hoursHtml = templateClone.find('#details');
      const days = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];
      let hasPartsHours = !1; let hasServiceHours = !1; let hasLeasingHours = !1; let
        hasSalesHours = !1;
      ...
      const isOpen = $.fn.isOpen(details); let
        isOpenHtml = '';
      ...
      tmpPinList.forEach((pin) => {
        ...
        const isOpen = $.fn.isOpen(pin); let
          isOpenHtml = '';
      ...
      $assist = 0, $full = 0, $rental = 0, $partsService = 0, $parts = 0, $premium = 0, $uptime = 0;
      ...
      const nearbyHtml = $('.nearby-pins-select').empty(); const tmpPinList = []; const
        sorted = $.fn.sortedPins();
      $nearbyPins.forEach((pin) => {
        tmpPinList.push($.grep(sorted, (v, i) => v.IDENTIFIER_VALUE === pin)[0]);
      ...
      const dealerId = $(evt).attr('data-dealerid'); const dealerZipcode = $(evt).attr('data-postalcode');
      const dealerEmail = $(evt).attr('data-dealeremail'); const dealerName = $(evt).attr('data-name');
      ...
      $('.partsasist-form').toggle(), $('.partsasist-form h3').text(dealerName), dealerFormTemplate.find('input[name="Dealercode"]').val(dealerId), dealerFormTemplate.find('input[name="SelectedBrand"]').val($brandOptionSelected), dealerFormTemplate.find('input[name="DealerPartsEmail"]').val(dealerEmail), dealerFormTemplate.find('input[name="Postalcode"]').val(dealerZipcode), $('#select-form').html(dealerFormTemplate.html()), $('.ajax').each(function () {
        const frm = $(this); const
          frmId = frm.attr('id');
        $(this).find('.ajaxSitecoreEloquaSubmit').each(function () {
          let redirectURL = '';
          $("input[name='redirectURL']").length && (redirectURL = `, '${$("input[name='redirectURL']").val()}'`), $(this).attr('href', `javascript:void(submitSitecoreEloquaForm('${frmId}'${redirectURL}));`);
      ...
      $(this).next('.accordion-panel').slideToggle('slow'), $(this).find('.toggle-arrow').toggleClass('close'), document.getElementById('share-link').select();
    }), $(document).on('click', '#set-dealer', function (eventObject) {
      console.log('inside set dealer');
      const pinId = $(this).attr('data-pin'); const
