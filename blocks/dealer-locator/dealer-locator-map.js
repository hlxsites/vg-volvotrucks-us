!(function e(t, n, r) {
  ...
  1: [function (require, module, exports) {
    $('.slider-arrow').click(function () {
      $(this).hasClass('show') ? ($(window).width() <= 700 ? $('.slider-arrow, .sidebar, .sidebar-legend').animate({
        left: '+=80%',
      }, 200, () => {}) : $('.slider-arrow, .sidebar, .sidebar-legend').animate({
        left: '+=483',
      }, 200, () => {}),
      $(this).removeClass('show').addClass('hide').find('i')
        .removeClass('fa fa-angle-right')
        .addClass('fa fa-angle-left')) : ($(window).width() <= 700 ? $('.slider-arrow, .sidebar, .sidebar-legend').animate({
        left: '-=80%',
      }, 200, () => {}) : $('.slider-arrow, .sidebar, .sidebar-legend').animate({
        left: '-=483',
      }, 200, () => {}),
    ...
    $viewingPin = null,
    $pinIcon = 'pin.svg',
    $meIcon = '/images/dealer-locator/volvo/ME.svg',
    $panes = [],
    $lastPane = '',
    ...
    $.fn.loadPins = function () {
      ...
      $nearbyPins = [],
      $markers = [],
      window.locatorConfig.dataSource || (window.locatorConfig.dataSource = '/simpleprox.ashx?https://mvservices.na.volvogroup.com/Dualbrand_US_DealerJSON.ashx'),
      $.ajax({
        ...
        success(data) {
                ...
                if ($country.states.hasOwnProperty(state)) {
                  ...
                  for (const dealer in $state.dealers) {
                    if ($state.dealers.hasOwnProperty(dealer)) {
                      if ($dealer = $state.dealers[dealer],
                      $dealer.services) {
                        for (const service in $dealer.services) {
                          $dealer.services.hasOwnProperty(service) && ($service = $dealer.services[service],
                          $service.toLowerCase().indexOf('certified uptime') > -1 && ($dealer.isCertifiedUptimeCenter = !0,
                          $electricDealer = !1),
                          $service.toLowerCase().indexOf('certified') > -1 && ($dealer.isCertifiedCenter = !0),
                          $service.toLowerCase().indexOf('premium used') > -1 && ($dealer.isPremiumUsedTruckDealer = !0,
                          $service = 'Premium Certified Used Truck Dealer'),
                          $service.toLowerCase().indexOf('select part store') > -1 && ($dealer.isPartsAssist = !0,
                      ...
                      if ($dealer.isCertifiedUptimeCenter) {
                        var pinIcon = {
                          url: '/images/dealer-locator/volvo/volvo-pin-uptime.svg',
                        ...
                        if ($electricDealer === !0 || $dealer.services && Object.values($dealer.services).includes('Volvo Certified EV Dealer')) {
                          var pinIcon = {
                            url: '/images/dealer-locator/volvo/volvo-pin-uptime-electric.svg',
                            scaledSize: new google.maps.Size(17, 23),
                            origin: new google.maps.Point(0, 0),
                            anchor: new google.maps.Point(0, 0),
                          };
                        ...
                        var pinIcon = {
                          url: '/images/dealer-locator/volvo/volvo-pin-dealer-electric.svg',
                          scaledSize: new google.maps.Size(17, 23),
                          origin: new google.maps.Point(0, 0),
                          anchor: new google.maps.Point(0, 0),
                        };
                        ...
                        var pinIcon = {
                          url: '/images/dealer-locator/volvo/volvo-pin-dealer.svg',
                          scaledSize: new google.maps.Size(17, 23),
                          origin: new google.maps.Point(0, 0),
                          anchor: new google.maps.Point(0, 0),
                        };
                      ...
                      $marker.addListener('click', function () {
                        ...
                        if ($map.panTo(marker.position),
                        'sidebar-select-pins' != $lastPane) {
                          $markers.forEach((marker) => {
                            let details = $.fn.getPinById(marker.ID);
          ...
          const viewMarker = $.fn.getPinById(markerId);
    ...
    $.fn.getHours = function (dealer) {
      let hours = null;
      return dealer.hours.Parts && (hours = dealer.hours.Parts),
    ...
    $.fn.isOpen = function (dealer, time) {
      ...
      if (hours) {
        ...
        if (todayAtDealer) {
          ...
          endTime.toLowerCase() == 'midnight' && (endTime = '11:59 PM'),
          startTime.toLowerCase().indexOf('24') > -1 && (startTime = '12:00 AM'),
          endTime.toLowerCase().indexOf('24') > -1 && (endTime = '11:59 PM'),
          startTime.toLowerCase() == 'noon' && (startTime = '12:00 PM'),
          endTime.toLowerCase() == 'noon' && (endTime = '12:00 PM');
          ...
          start.setSeconds(0);
          const end = new Date();
          return end.setFullYear(time.getFullYear()),
          ...
          ~endTime.toLowerCase().indexOf('am') && (console.log('end date is AM, add 1 day'),
          end.setDate(time.getDate() + 1)),
          moment.tz(zone).isBetween(start, end) ? (hours = Math.abs(time - end) / 36e5,
          hours < 1 && (closeSoon = !0),
          {
            open: !0,
            endTime: end,
    ...
    $.fn.canDetermineHours = function (pin) {
      let hours = null;
      return pin.hours.Parts && (hours = pin.hours.Parts),
      !hours && pin.hours.Service && (hours = pin.hours.Service),
      !hours && pin.hours.Leasing && (hours = pin.hours.Leasing),
      !!hours;
    }
    ,
    $.fn.renderPinDirections = function (markerId) {
      let markerDetails; var templateClone = $($('#sidebar-directions').clone(!0).html()); var
        templateClone = $($('#sidebar-direction-list').clone(!0).html());
      for ($('.add-directions').attr('data-id', markerId),
      i = 0; i < $sortedPins.length; i++) $sortedPins[i].IDENTIFIER_VALUE == markerId && (markerDetails = $sortedPins[i]);
      $('.main-header').css('display', 'none'),
      $('.main-directions').css('display', 'block'),
      $('.go-back').css('display', 'none'),
      $origin = $currentAddress,
      $origin && $origin != '' || ($origin = `${$location[0]},${$location[1]}`,
      ...
      $('.from-directions input').val() && ($origin = $('.from-directions input').val()),
      $directionsObject = {
        origin: $origin,
      ...
      $directionsService.route($directionsObject, (result, status) => {
        status == 'OK' && ($directionsDisplay.setMap($map),
      ...
      return templateClone.find('#gmaps-link').attr('onclick', `window.open("https://www.google.com/maps/dir/${$origin}/${$destination}${waypointDecodeUrl}", "_blank")`),
      templateClone;
    }
    ,
    $.fn.renderPinDetails = function (markerId) {
      let markerDetails; const
      ...
      markerDetails.PARTS_EMAIL ? templateClone.find('#partsasist-button').attr('data-dealeremail', markerDetails.PARTS_EMAIL.toLowerCase()) : templateClone.find('#partsasist-button').attr('data-dealeremail', markerDetails.EMAIL_ADDRESS.toLowerCase()),
      ...
      markerDetails.WEB_ADDRESS ? templateClone.find('#website').html(`<a href="${$.fn.formatWebAddress(markerDetails.WEB_ADDRESS)}" target="_blank">${$.fn.formatWebAddress(markerDetails.WEB_ADDRESS)}</a>`) : templateClone.find('#website .controls').css('display', 'none'),
      markerDetails.EMAIL_ADDRESS && templateClone.find('#email').html(`<a href="mailto:${markerDetails.EMAIL_ADDRESS.toLowerCase()}"><img src="/images/dealer-locator/volvo/Mail.png" />${markerDetails.EMAIL_ADDRESS.toLowerCase()}</a>`),
      templateClone.find('.detail-website a').attr('href', $.fn.formatWebAddress(markerDetails.WEB_ADDRESS)),
      templateClone.find('.detail-email').html(`<a href="mailto:${markerDetails.EMAIL_ADDRESS.toLowerCase()}"><img src="/images/dealer-locator/volvo/Mail-2.png" />Email</a>`),
      ...
      templateClone.find('.detail-call').html(`<a href="tel:${markerDetails.REG_PHONE_NUMBER}"><img src="/images/dealer-locator/volvo/Phone-2.png" />Call</a>`),
      ...
      driversId && !driversId.find('li').length || newdriverAmenities.length == 0 ? templateClone.find('.header-driver-title').hide() : templateClone.find('.header-driver-title').show(),
      $.isEmptyObject(markerDetails.services) && templateClone.find('.header-title').css('display', 'none');
      let hours = null;
      ...
      if (markerDetails.hours.Parts) {
        ...
        var added = 0;
        $.each(hours, (i, item) => {
          if (hours[i].Start.toLowerCase().indexOf('24 hours') >= 0 || hours[i].Start.toLowerCase().indexOf('closed') >= 0) hours[i].Start = '';
          else if (hours[i].Start.toLowerCase().length <= 0) return;
          $('<div/>', {
      ...
      if (markerDetails.hours.Sales && (hours = markerDetails.hours.Sales,
      hours[0].Start.length > 0)) {
        $('<div/>', {
      ...
      return hasPartsHours || hasServiceHours || hasLeasingHours || hasSalesHours || (isOpenHtml = 'Call'),
      templateClone.find('#hours div').html(`${isOpenHtml} <span class="toggle-arrow"></span>`),
      $.isEmptyObject(hours) && templateClone.find('.toggle-arrow').css('display', 'none'),
    ...
    $.fn.renderAddDirectionsPin = function (marker, details) {
      ...
      templateClone.find('.detail-call').html(`<a href="tel:${pin.REG_PHONE_NUMBER}"><img src="/images/dealer-locator/volvo/Phone-2.png" />Call</a>`),
      $('<div/>', {
        html: templateClone,
      ...
      for (let i = 0; i < $markers.length; i++) {
        for (let k = 0; k < $pins.length; k++) {
          if ($pins[k].IDENTIFIER_VALUE == $markers[i].id && $.fn.showPin($pins[k])) {
            if ($markers[i].setMap($map),
            $.fn.isWaypoint($pins[k].waypoint)) var pinIcon = $.fn.drawPin('-', 38, 38, '808080');
            else var pinIcon = $.fn.drawPin('+', 38, 38, '4174C4');
            $markers[i].setIcon(pinIcon);
          }
        }
      }
    ...
    $.fn.switchSidebarPane = function (id, e) {
      ...
      e && id == 'sidebar-pin' && (content = $.fn.renderPinDetails(markerId)),
      e && id == 'sidebar-directions' || id == 'add-directions-return' || id == 'sidebar-direction-list' ? (content = $.fn.renderPinDirections(markerId),
      $('.sidebar-content').addClass('direction-content'),
      $('.go-back').css('display', 'none'),
      $('.sidebar-legend').hide(),
      $(window).width() < 992 && ($('.sidebar').css('overflow', 'scroll'),
      $('.add-directions').text('Recalculate')),
      id == 'add-directions-return' && (forceRefresh = !0)) : id.indexOf('sidebar-select-pins') >= 0 ? ($.fn.setupAddDirectionsView(),
      content = $(`#${id}`).html(),
      $('.main-directions').css('display', 'none'),
      $('.main-header').css('display', 'none')) : ($('.main-directions').css('display', 'none'),
      $('.main-header').css('display', 'block'),
      $.fn.clearDirections(),
      $map.setZoom(8)),
      $lastPane && $(`#d-${$lastPane}`).css('display', 'none'),
      id == 'sidebar-directions' || id == 'sidebar-direction-list' ? $('.go-back').css('display', 'none') : id == 'sidebar-filter' ? ($('.go-back').css('display', 'block'),
      $('.go-back').css('background', 'none')) : id == 'sidebar-pin' ? ($('.go-back').css('display', 'block'),
      $('.go-back').css('background', '#202A44')) : $('.go-back').css('display', 'none'),
      document.body.scrollTop = 0,
      $('.sidebar').scrollTop(0),
      id += e != null ? `-${markerId}` : '';
      let foundPane = !1;
      $panes.forEach((pane) => {
        pane == id && (id.indexOf('directions') >= 0 || id.indexOf('direction') >= 0 ? ($(`#d-${id}`).remove(),
        $('.go-back').css('display', 'none')) : id.indexOf('sidebar-pin') >= 0 && id.indexOf(`sidebar-pin-${markerId}`) >= 0 ? $(`#d-${id}`).remove() : ($(`#d-${id}`).css('display', 'block'),
        foundPane = !0));
      }),
      foundPane || ($('<div/>', {
        id: `d-${id}`,
      ...
      $panes.push(id)),
      forceRefresh && $(`#d-${id}`).html(content),
      $('.sidebar').css('left') != '0px' && ($(window).width() <= 700 ? $('.slider-arrow, .sidebar').animate({
        left: '+=80%',
      }, 200, () => {}) : $('.slider-arrow, .sidebar').animate({
        left: '+=408',
      }, 200, () => {}),
    ...
    $.fn.filterRadius = function () {
      ...
      for (var radius = $.fn.milesInMeters($('#range').val()), bounds = new google.maps.LatLngBounds(), k = 1, i = 0; i < $markers.length; i++) {
        if (google.maps.geometry.spherical.computeDistanceBetween($markers[i].getPosition(), $me.getPosition()) < radius) {
          ...
          var pin = $.grep($pins, (v, b) => v.IDENTIFIER_VALUE === $markers[i].ID)[0];
          $.fn.showPin(pin) ? ($markers[i].setMap($map),
          $markers[i].setZIndex(2),
          $nearbyPins.push($markers[i].ID),
          k++) : $markers[i].setMap(null);
        } else {
          var pin = $.grep($pins, (v, b) => v.IDENTIFIER_VALUE === $markers[i].ID)[0];
          if ($.fn.showPin(pin) && $markers[i].setMap($map),
          $myDealer != null && $myDealer.IDENTIFIER_VALUE == $markers[i].id) var pinIcon = $.fn.drawPin('', 43, 63, '328E04');
          else if (details.isCertifiedUptimeCenter) {
            var pinIcon = {
              url: '/images/dealer-locator/volvo/volvo-pin-uptime.svg',
              scaledSize: new google.maps.Size(17, 23),
              origin: new google.maps.Point(0, 0),
    ...
    $.fn.radiusChange = function () {
      ...
      $.fn.filterRadius(),
      $value < 100 ? $map.fitBounds($radius.getBounds()) : $map.setZoom(8);
    }
    ,
    ...
    $.fn.sortedPins = function () {
      ...
      return $sortedPins = $pins,
      $sortedPins.sort((a, b) => parseFloat(a.distance) - parseFloat(b.distance)),
      $sortedPins.filter((i, n) => {
        $.fn.showPin(i);
      }),
      $sortedPins;
    }
    ...
    $.fn.tmpPins = function (tmpPinList) {
      let pinIndex = 1;
      const nearbyHtml = $('.nearby-pins').empty();
      tmpPinList.forEach((pin) => {
        ...
        for (i = 0; i < $markers.length; i++) {
          if ($markers[i].id == pin.IDENTIFIER_VALUE) {
            if (marker = $markers[i],
            $myDealer || ($myDealer = {
              IDENTIFIER_VALUE: $.fn.getCookie('my-dealer'),
            }),
            $myDealer.IDENTIFIER_VALUE == pin.IDENTIFIER_VALUE) {
              ...
              var pinIcon = '/images/dealer-locator/volvo/volvo-pin-uptime.svg';
              var pinIcon2 = {
                url: '/images/dealer-locator/volvo/volvo-pin-uptime.svg',
                scaledSize: new google.maps.Size(17, 23),
                origin: new google.maps.Point(0, 0),
              ...
              if ($electricDealer === !0 || pin.services && Object.values(pin.services).includes('Volvo Certified EV Dealer')) {
                var pinIcon = '/images/dealer-locator/volvo/volvo-pin-uptime-electric.svg';
                var pinIcon2 = {
                  url: '/images/dealer-locator/volvo/volvo-pin-uptime-electric.svg',
                  scaledSize: new google.maps.Size(17, 23),
                  origin: new google.maps.Point(0, 0),
              ...
              var pinIcon = '/images/dealer-locator/volvo/volvo-pin-dealer-electric.svg';
              var pinIcon2 = {
                url: '/images/dealer-locator/volvo/volvo-pin-dealer-electric.svg',
                scaledSize: new google.maps.Size(17, 23),
                origin: new google.maps.Point(0, 0),
              ...
              var pinIcon = '/images/dealer-locator/volvo/volvo-pin-dealer.svg';
              var pinIcon2 = {
                url: '/images/dealer-locator/volvo/volvo-pin-dealer.svg',
                scaledSize: new google.maps.Size(17, 23),
                origin: new google.maps.Point(0, 0),
    ...
    $.fn.filterNearbyPins = function () {
      ...
      $('#filterUptime, #filterUptimeMobile ').unbind().bind('click', function (e) {
        ...
        filteredArray.length == 0 && ($('.nearby-pins').empty(),
        $('.no-dealer-text').show());
        const newList = [];
        let pins = [];
        return pins = $pins,
        pins && jQuery.grep(pins, (el) => {
          jQuery.inArray(el, filteredArray) == -1 && newList.push(el);
        }),
        toggled ? ($(this).css('background', '#484A4E'),
        $('.no-dealer-text').hide(),
        $.fn.tmpPins(tmpPinList),
        newList.forEach((pin) => {
          if (!$.fn.showPin(pin)) return !0;
          for (i = 0; i < $markers.length; i++) {
            $markers[i].id == pin.IDENTIFIER_VALUE && (marker = $markers[i],
            marker.setMap($map));
          }
        }),
        document.getElementById('filterElectricDealer').style.pointerEvents = 'auto',
        ...
        document.getElementById('electric-tag').removeAttribute('title', $hoverText)) : ($(this).css('background', '#808080'),
      ...
      $('#filterElectricDealer, #filterElectricDealerMobile ').unbind().bind('click', function (e) {
        ...
        filteredDealerArray.length == 0 && ($('.nearby-pins').empty(),
        $('.no-dealer-text').show());
        const newList = [];
        let pins = [];
        return pins = $pins,
        ...
        document.getElementById('filterUptimeMobile').style.pointerEvents = 'auto',
        document.getElementById('dealer-tag').removeAttribute('title', $hoverText),
        document.getElementById('uptime-tag').removeAttribute('title', $hoverText)) : ($(this).css('background', '#808080'),
      ...
      $isAsist && $('.panel-footer').html(`Showing ${$nearbyPins.length} ${$brandOptionSelected == 'dual' ? '' : $brandOptionSelected} locations`)),
      ...
      tmpPinList.forEach((pin) => {
        ...
        for (i = 0; i < $markers.length; i++) {
          if ($markers[i].id == pin.IDENTIFIER_VALUE) {
            ...
            $myDealer.IDENTIFIER_VALUE == pin.IDENTIFIER_VALUE) var pinIcon = $.fn.drawPin('', 43, 63, '328E04');
            else {
              const details = $.fn.getPinById(pin.ID);
              if (details.isCertifiedUptimeCenter) {
                var pinIcon = '/images/dealer-locator/volvo/volvo-pin-uptime.svg';
                ($electricDealer === !0 || details.services && Object.values(details.services).includes('Volvo Certified EV Dealer')) && (pinIcon = '/images/dealer-locator/volvo/volvo-pin-uptime-electric.svg');
              } else if ($electricDealer === !0 || details.services && Object.values(details.services).includes('Volvo Certified EV Dealer')) var pinIcon = '/images/dealer-locator/volvo/volvo-pin-dealer-electric.svg';
              else var pinIcon = '/images/dealer-locator/volvo/volvo-pin-dealer.svg';
            }
            marker.setIcon(pinIcon),
            marker.pinIndex = pinIndex,
            templateClone.find('#marker').attr('src', pinIcon),
    ...
    $.fn.getDistanceInKm = function ($b) {
      return $location ? ($R = 6371,
      $dLat = $.fn.deg2rad($b[0] - $location[0]),
      $dLon = $.fn.deg2rad($b[1] - $location[1]),
      $a = Math.sin($dLat / 2) * Math.sin($dLat / 2) + Math.cos($.fn.deg2rad($location[0])) * Math.cos($.fn.deg2rad($b[0])) * Math.sin($dLon / 2) * Math.sin($dLon / 2),
    ...
    $.fn.setAddress2 = function () {
      return address2 = $('#location2').val(),
      address2 ? ($geocoder = new google.maps.Geocoder(),
    ...
    $.fn.setAddress = function () {
      return $(window).width() < 992 && (address = address2),
    ...
    $.fn.setLocation = function () {
      navigator.geolocation ? navigator.geolocation.getCurrentPosition((position) => {
        ...
        $geocoder.geocode({
          ...
          status == google.maps.GeocoderStatus.OK ? (results[6] ? address = results[0].formatted_address : results[0] ? address = results[5].formatted_address : console.log('no results found'),
          address && ($('.sidebar #location').val(address),
          $currentAddress = address,
          $('#location2').val(address),
          $currentAddress = address)) : console.log(`geocode failed due to: ${status}`);
        });
      }, () => {
        console.log('error with navigator'),
        $.fn.handleLocationError(!0);
      }) : (console.log('could not determine location'),
      $.fn.handleLocationError(!1));
    }
    ,
    $.fn.currentFilter = function () {
      return $current = $('#filter-options input:checked').first(),
      void 0 !== $current.get(0) ? $('#filter-options input:checked').first().attr('id') : '';
    }
    ,
    $.fn.currentFilterHumanReadable = function () {
      return $current = $('#filter-options input:checked').first(),
      void 0 !== $current.get(0) ? $('#filter-options input:checked').first().attr('value') : '';
    }
    ,
    $.fn.setMyDealer = function (dealer) {
      ...
      dealer) {
        ...
        let hours = {};
        $myDealer.hours.Parts && (hours = $myDealer.hours.Parts),
        $.isEmptyObject(hours) && $myDealer.hours.Service && (hours = $myDealer.hours.Service),
        $.isEmptyObject(hours) && $myDealer.hours.Leasing && (hours = $myDealer.hours.Leasing),
        ...
        $.fn.setCookie('my-dealer', $myDealer.IDENTIFIER_VALUE, 3650),
        $.fn.setCookie('my-dealer-name', $myDealer.COMPANY_DBA_NAME, 3650),
        $.fn.setCookie('my-dealer-hours', JSON.stringify(hours), 3650),
        $.fn.setCookie('my-dealer-number', $myDealer.REG_PHONE_NUMBER, 3650),
        $.fn.setCookie('my-dealer-coords', `${$myDealer.MAIN_LATITUDE},${$myDealer.MAIN_LONGITUDE}`, 3650),
        $.fn.setCookie('my-dealer-city', $myDealer.MAIN_ADDRESS_LINE_1_TXT, 3650),
        $.fn.setCookie('my-dealer-city', $myDealer.MAIN_ADDRESS_LINE_2_TXT, 3650),
        $.fn.setCookie('my-dealer-city', $myDealer.MAIN_CITY_NM, 3650),
        $.fn.setCookie('my-dealer-state', $myDealer.MAIN_STATE_PROV_CD, 3650),
        $.fn.setCookie('my-dealer-zip', $myDealer.MAIN_POSTAL_CD, 3650),
      ...
      $dealerEvent = new CustomEvent('my-dealer', {
        detail: $eventData,
      }),
    ...
    $.fn.drawPin = function (text, width, height, color) {
      return text || (text = ''),
      color == '328E04' && $.fn.isMack(),
      color == '3F62A5' && $.fn.isMack() && (color = '85754d'),
      `data:image/svg+xml,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20width%3D%22${width}%22%20height%3D%22${height}%22%20viewBox%3D%220%200%2038%2038%22%3E%3Cpath%20fill%3D%22%23${color}%22%20stroke%3D%22%23ccc%22%20stroke-width%3D%22.5%22%20d%3D%22M34.305%2016.234c0%208.83-15.148%2019.158-15.148%2019.158S3.507%2025.065%203.507%2016.1c0-8.505%206.894-14.304%2015.4-14.304%208.504%200%2015.398%205.933%2015.398%2014.438z%22%2F%3E%3Ctext%20transform%3D%22translate%2819%2018.5%29%22%20fill%3D%22%23fff%22%20style%3D%22font-family%3A%20Arial%2C%20sans-serif%3Bfont-weight%3Abold%3Btext-align%3Acenter%3B%22%20font-size%3D%2212%22%20text-anchor%3D%22middle%22%3E${text}%3C%2Ftext%3E%3C%2Fsvg%3E`;
    }
    ,
    $.fn.handleLocationError = function (browserHasGeolocation, infoWindow, pos) {
      browserHasGeolocation ? ($('.loading-overlay').css('display', 'none'),
      $('.waiting-overlay').css('display', 'block')) : alert("Error: Your browser doesn't support geolocation.");
    }
    ,
    ...
    $.fn.getCurrentRouteTimeInHours = function () {
      ...
      $currentTime = null,
      $directionResults && ($directionResults.routes && $directionResults.routes.length == 1 ? $currentRoute = $directionResults.routes[0] : $directionResults.routes && $directionResults.routes.length > 1 && ($referencePoint = $('.adp-listsel').first(),
      $currentRoute = $directionResults.routes[$referencePoint.attr('data-route-index')])),
      $currentRoute && ($currentTime = $currentRoute.legs[0].duration.value / 60 / 60),
      $currentTime;
    }
    ,
    ...
    $.fn.getUrlParameter = function (sParam) {
      let sParameterName; let i; const sPageURL = window.location.search.substring(1); const
        sURLVariables = sPageURL.split('&');
      for (i = 0; i < sURLVariables.length; i++) {
        if (sParameterName = sURLVariables[i].split('='),
        sParameterName[0] === sParam) return void 0 === sParameterName[1] || decodeURIComponent(sParameterName[1]);
      }
    }
    ,
    ...
    $.fn.registration = function (evt) {
      ...
      $('.ajax').each(function () {
        ...
        $(this).find('.ajaxEloquaSubmit').each(function () {
          let redirectURL = '';
          $jq1("input[name='redirectURL']").length && (redirectURL = `, '${$jq1("input[name='redirectURL']").val()}'`),
    ...
    $(window).on('resize', () => {
      $(window).width() < 992 && $('.direction-content') ? ($('.sidebar').css('overflow', 'scroll'),
      $('.add-directions').text('Recalculate')) : ($('.sidebar').css('overflow', 'hidden'),
      $('.add-directions').text('Recalculate Directions')),
      $('#location').val() && $('#location2').val($('#location').val()),
      $('#location2').val() && $('#location').val($('#location2').val());
    }),
    $(document).on('click', '.sidebar-content #filter-options input[type="checkbox"]', function (e) {
      return $clicked = $(this),
      $('.sidebar-content #filter-options input[type="checkbox"]:checked').length < 1 ? void $clicked.prop('checked', !0) : ($('.sidebar-content #filter-options input[type="checkbox"]').each(function (i) {
        $filter = $(this),
        $clicked.get(0) !== $filter.get(0) && $filter.prop('checked', !1);
      }),
    ...
    $(document).on('click', '#set-dealer', function (eventObject) {
      ...
      const pin = $.fn.getPinById(pinId);
      $.fn.setMyDealer(pin),
      $(this).find('#set-dealer').html('<img src="/images/dealer-locator/volvo/VOLVO-MACK_Dealer-Locator-Icons/VOLVO-MACK_Dealer-Locator-Icons/Vector-3.svg" />');
    }),
